...
1. `ViewerApp.cpp` 当前有 1165 行，其中 `ViewerApp::run()` 独占 908 行；它仍同时
   ...
   CTest 的工程护栏会校验上述行数，并以 1165 / 908 / 2250 / 763 行分别作为
   ...
   `ViewerCameraConfig`；首启工作台窗口几何已由可测的 `ViewerWorkbenchLayout` 计算；
   `AppConfigViewerToml` 已按 TOML section 解析 viewer/runtime 域，loader 只编排
   输入、CSV 与路径解析；
