...
1. `ViewerApp.cpp` 当前有 911 行，其中 `ViewerApp::run()` 独占约 670 行。运行时资源、
   ...
   调度与错误边界。`UiRoot.cpp` 仍有 890 行（视口画布绘制已拆分到
   `ViewportCanvas.cpp`，TIA-92 把顶栏/状态栏/快捷键总览/命令面板拆到 AppChrome.cpp），剩余的 docking
   编排与面板绘制仍集中在一个文件。工程护栏以 912 / 716 / 890 / 763 行分别约束 `ViewerApp.cpp`、
