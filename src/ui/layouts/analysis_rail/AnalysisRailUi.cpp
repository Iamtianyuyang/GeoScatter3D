...
static AnalysisRailUiState s_rail_ui;
...
void toggle_drawer(
    AnalysisRailUiState& rail,
    const AnalysisDrawer drawer
) {
    rail.open_drawer =
        rail.open_drawer == drawer
            ? AnalysisDrawer::kNone
            : drawer;
}
...
void draw_drawer_header(
    ...
    if (dark_button("×##CloseDrawer", ImVec2(24.0f * scale, 24.0f * scale))) {
        rail.open_drawer = AnalysisDrawer::kNone;
    }
    ImGui::Separator();
}
...
void draw_data_drawer(
    ...
    draw_drawer_header("数据", s_rail_ui, scale);
...
void draw_appearance_drawer(
    ...
    draw_drawer_header("外观", s_rail_ui, scale);
    auto& settings =
...
void draw_measure_drawer(
    ...
    draw_drawer_header("测量", s_rail_ui, scale);
    auto& measurement =
...
void draw_views_drawer(
    ...
    draw_drawer_header("视图", s_rail_ui, scale);
...
void draw_system_drawer(
    ...
    draw_drawer_header("系统", s_rail_ui, scale);
...
void draw_rail(
    ...
    const std::array<AnalysisDrawer, 5> drawers{
        AnalysisDrawer::kData,
        AnalysisDrawer::kAppearance,
        AnalysisDrawer::kMeasure,
        AnalysisDrawer::kViews,
        AnalysisDrawer::kSystem
    };
...
void draw_navigation_preview(
    ...
    if (map.valid && map.texture_descriptor != kNullTextureHandle) {
        draw->AddImage(
            static_cast<ImTextureID>(
                static_cast<ImTextureID>(map.texture_descriptor)
            ),
...
AnalysisRailFrameResult draw_analysis_rail_layout(
    ...
    if (rail.open_drawer != AnalysisDrawer::kNone &&
        ...
        if (layout.drawer_width > 1.0f) {
            ...
            if (begin_region(
                ...
                if (rail.drawer_anim > 0.96f && view_index >= 0) {
                    ...
                    case AnalysisDrawer::kData:
                        draw_data_drawer(state, actions, scale);
                        break;
                    case AnalysisDrawer::kAppearance:
                        ...
                        break;
                    case AnalysisDrawer::kMeasure:
                        draw_measure_drawer(state, view_index, scale);
                        break;
                    case AnalysisDrawer::kViews:
                        draw_views_drawer(
                            state,
                            actions,
                            view_index,
