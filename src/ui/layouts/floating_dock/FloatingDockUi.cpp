    ...
    if (ImGui::Begin("##FloatingDockFileChip", nullptr, kOverlayWindowFlags)) {
        ...
        if (hovered) {
            ...
            if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
                toggle_card(s_dock_ui, DockCard::kData);
            }
        }
    }
    ImGui::End();
...
void draw_camera_pill(
    ...
    if (ImGui::Begin("##FloatingDockCameraPill", nullptr, kOverlayWindowFlags)) {
        ...
        if (draw_camera_pill_buttons(
                view,
                actions,
                dock.open_card == DockCard::kCrosshairStyle,
                s
            )) {
            toggle_card(dock, DockCard::kCrosshairStyle);
        }
    }
    ImGui::End();
    ImGui::PopStyleVar();
...
void draw_dock(
    ...
    if (ImGui::Begin("##FloatingDock", nullptr, kOverlayWindowFlags)) {
        ...
        static const DockItemDesc items[] = {
            {"##dock_views",   "视图", icon_monitor, DockCard::kViews},
            {"##dock_measure", "测量", icon_ruler,   DockCard::kMeasure},
            {"##dock_layers",  "图层", icon_layers,  DockCard::kLayers},
            {"##dock_props",   "属性", icon_sliders, DockCard::kAppearance},
            {"##dock_settings", "设置", icon_settings, DockCard::kSettings},
        };
        bool first = true;
        for (const auto& item : items) {
            ...
            first = false;
            const int badge =
                item.card == DockCard::kViews ? visible_views : 0;
            draw_dock_item(
                ctx,
                item,
                s_dock_ui.open_card == item.card,
                badge,
                s_dock_ui
            );
        }
...
void draw_card_views(
    ...
    if (active_index >= 0 &&
        ...
        if (widgets::Button(
            ...
            s_dock_ui.open_card = DockCard::kNone;
        }
        ImGui::Spacing();
        const bool navigation_floating =
            s_dock_ui.navigation_map_floating;
        if (widgets::Button(
                navigation_floating
                    ? "收回导航图"
                    : "弹出导航图为悬浮窗口",
            ...
            s_dock_ui.navigation_map_floating = !navigation_floating;
        }
        ImGui::Spacing();
        if (s_dock_ui.navigation_map_floating) {
            ScopedFont font(small_font());
            ImGui::PushStyleColor(
                ImGuiCol_Text,
                to_u32(palette::kTextDim, 205)
...
void draw_card_settings(
    ...
    switch_row("性能 HUD", &s_dock_ui.show_perf_hud, s);
    ...
    if (widgets::Button(
        ...
        s_dock_ui.open_card = DockCard::kNone;
        s_dock_ui.anim_card = DockCard::kNone;
        s_dock_ui.card_anim = 0.0f;
    }
    if (widgets::Button(
            "方案 C · 暗色分析舱",
            widgets::ButtonVariant::kSecondary,
...
const char* card_title(DockCard card) {
    switch (card) {
        case DockCard::kViews:      return "多视口";
        case DockCard::kMeasure:    return "测量工具";
        case DockCard::kLayers:     return "图层";
        case DockCard::kAppearance: return "点云外观";
        case DockCard::kSettings:   return "设置";
        case DockCard::kData:        return "数据";
        case DockCard::kPerformance: return "性能";
        case DockCard::kCrosshairStyle:
            return "准星样式";
        default:                               return "";
    }
}

bool card_allows_vertical_scroll(const DockCard card)
{
    switch (card) {
        case DockCard::kMeasure:
        case DockCard::kLayers:
        case DockCard::kAppearance:
        case DockCard::kSettings:
        case DockCard::kData:
            return true;
        case DockCard::kViews:
        case DockCard::kPerformance:
        case DockCard::kCrosshairStyle:
        case DockCard::kNone:
            return false;
    }
...
void draw_dock_card(
    ...
    auto& dock = s_dock_ui;
    if (dock.anim_card == DockCard::kNone ||
        dock.card_anim <= 0.0f) {
        return;
    }
    const float s = ctx.s;
    const bool closing = dock.open_card == DockCard::kNone;
    ...
    const bool top_left =
        card == DockCard::kData ||
        card == DockCard::kPerformance;
    const bool top_right =
        card == DockCard::kCrosshairStyle;
    float card_width = DockMetrics::kCardWidth;
    if (card == DockCard::kMeasure ||
        card == DockCard::kData ||
        card == DockCard::kPerformance) {
        card_width = 400.0f;
    } else if (top_right) {
    ...
    if (top_left) {
        const float y_offset =
            card == DockCard::kData ? 76.0f : 104.0f;
        card_pos = ImVec2(
            ctx.work_pos.x + DockMetrics::kSideMargin * s,
    ...
    if (ImGui::Begin("##FloatingDockCard", nullptr, flags)) {
        ...
        if (allow_vertical_scroll) {
            ...
            static DockCard previous_scroll_card =
                DockCard::kNone;
...
void draw_hint_pill(const FrameCtx& ctx, DockUiState& dock) {
...
FloatingDockFrameResult draw_floating_dock_layout(
    ...
    // ── 卡片开合动画推进 ──
    const bool opening = dock.open_card != DockCard::kNone;
    ...
    if (ImGui::Begin(
        ...
        if (dock.open_card != DockCard::kNone &&
